...
ASSET_MAP = {
    "docs/dm/-/locations": "docs/assets/location",
    "docs/dm/-/npc":       "docs/assets/npc",
    "docs/monsters/-":     "docs/assets/monsters",
}
# --------------------------------------------------------------------
...
def resolve_bare_image_by_map(src: Path, root: Path, filename: str, mapping: dict[str, str]):
    ...
    if not asset_dir.is_dir():
        return None
    # look for exact name (case-insensitive)
    fl = filename.lower()
    candidates = [p for p in asset_dir.rglob("*") if p.is_file() and p.name.lower() == fl]
    return choose_best(candidates) if candidates else None
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Obsidian → Plain Markdown (Zettlr + MkDocs) — with Asset Mapping")
