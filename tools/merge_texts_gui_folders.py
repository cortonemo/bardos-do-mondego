...
- Add files or entire folders (optionally include subfolders)
...
class MergeApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.listbox = tk.Listbox(file_frame, height=14, selectmode=tk.EXTENDED, activestyle="dotbox")
        ...
        ttk.Button(btns, text="Adicionar ficheiros…", command=self.add_files).pack(fill="x", pady=2)
        ttk.Button(btns, text="Adicionar pasta…", command=self.add_folder).pack(fill="x", pady=2)

        subf_frame = ttk.Frame(btns)
        subf_frame.pack(fill="x", pady=(2,8))
        ttk.Checkbutton(subf_frame, text="Incluir subpastas", variable=self.include_subfolders).pack(anchor="w")
        ...
        # Progress bar
        self.progress = ttk.Progressbar(opts, orient="horizontal", mode="determinate", length=280)
    ...
    def add_folder(self):
        folder = filedialog.askdirectory(title="Seleciona a pasta de origem")
        if not folder:
            return
        ...
        if self.include_subfolders.get():
            ...
            for p in folder_path.glob("*"):
                if p.is_file() and p.suffix.lower() in ALLOWED_EXTS:
                    to_add.append(str(p))
        if not to_add:
            self.log_msg(f"[Info] Nenhum .txt/.md encontrado em: {folder_path}")
            return
        self._add_paths(to_add, announce_folder=folder_path)

    def _add_paths(self, paths, announce_folder=None):
        added = 0
        skipped = 0
        for p in sorted(set(paths), key=lambda x: Path(x).name.lower()):
        ...
        if announce_folder:
            self.log_msg(f"Adicionados {added} ficheiro(s) de '{announce_folder}'. Ignorados {skipped} duplicado(s).")
        else:
            self.log_msg(f"Adicionados {added} ficheiro(s). Ignorados {skipped} duplicado(s).")
    ...
    def _merge_and_split(self):
        ...
        # Prepare output
        part_idx = 1
        # If total fits in one part, keep single-base name; otherwise start with part01
        single_output = chunk_size >= total_bytes > 0
        current_path = out_dir / (f"{base_root}{base_ext}" if single_output else f"{base_root}_part{part_idx:02d}{base_ext}")
