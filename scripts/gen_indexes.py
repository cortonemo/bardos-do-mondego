...
# Folders to skip explicitly
SKIP_DIRS = {".obsidian", "site", "image DL", "__pycache__"}

# Folder names to human-readable titles
TITLE_MAP = {
    "dm": "DM",
    ".dm": "DM",
...
def write_index(dir_path: Path) -> None:
    ...
    subs = sorted(
        [d for d in dir_path.iterdir() if d.is_dir() and d.name not in SKIP_DIRS],
...
# Generate indexes for all subfolders under /docs
for d in DOCS.rglob("*"):
