## As Máscaras da Morte / The Masks of Death

| 🗓 Timeline / Cronologia | PT | EN |
| :-------------------- | :---------------------------------------------------- | :------------------------------------------------------- |
| **Data do Jogo** | 30 de Janeiro de 2025 | January 30th, 2025 |
| **Data In-Game** | 15 de Ready'reat, 998 CY (Semana 05) | 15th of Ready'reat, 998 CY (Week 05) |
| **Sessão #** | S05A | S05A |
| **DM** | André Maia | André Maia |
| **Arcos Enquadrados** | **🦑 Arco da Corrupção do Mal Elemental** | **🦑 Elemental Evil Corruption Arc** |

---
## 📖 Overview / Sumário

| Descrição | PT | EN |
| :--- | :--- | :--- |
| **Missão Principal** | O grupo é enviado para a vila comercial de **[Mistwood](mistwood.md)** para uma missão de reconhecimento e estabelecimento de rotas comerciais. A missão rapidamente se transforma num confronto direto com uma célula do **[Culto do Mal Elemental](cult_of_elemental_evil.md)**, liderada pela **Acólita Morvanya**, uma serva de [Nerull](nerull.md) (Deus da Morte). | The party is sent to the trading village of **[Mistwood](mistwood.md)** for a reconnaissance and trade route establishment mission. The mission quickly turns into a direct confrontation with a cell of the **[Cult of Elemental Evil](cult_of_elemental_evil.md)**, led by **Acolyte Morvanya**, a servant of [Nerull](nerull.md) (God of Death). |
| **Resultado** | O grupo derrota Morvanya, mas não antes de **[Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)** descobrir que a acólita é na verdade a sua melhor amiga perdida, **Manvea**. Para proteger os segredos do Culto, Manvea/Morvanya comete **suicídio** com veneno. O grupo regressa com uma **Tapeçaria** dos cultistas, que transformam em capas de culto. | The party defeats Morvanya, but not before **[Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)** discovers that the acolyte is actually her lost best friend, **Manvea**. To protect the Cult’s secrets, Manvea/Morvanya **commits suicide** with poison. The group returns with a **Tapestry** from the cultists, which they turn into cult cloaks. |

---
## 🎲 Key Events / Eventos Chave

| Evento | PT | EN |
| :--- | :--- | :--- |
| **O Chamado de Vaunia** | Durante o acampamento, **[Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)** manifesta um novo *companion* animal (um *doggo* chamado Boaz), recupera fragmentos de memória (os nomes **Manvea** e **Aurkan**) e percebe uma presença sombria que a segue. | During the camp, **[Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)** manifests a new animal *companion* (a *doggo* named Boaz), recovers memory fragments (the names **Manvea** and **Aurkan**), and perceives a shadowy presence following her. |
| **Confronto em Mistwood** | O grupo ataca a célula do culto numa área próxima ao pântano infestada por fungos. A Acólita **Morvanya** usa magia de sombras (escuridão) e invoca um *Bonegoblin* e um *Zombie*. | The party attacks the cult cell in a fungus-infested area near the swamp. Acolyte **Morvanya** uses shadow magic (darkness) and summons a *Bonegoblin* and a *Zombie*. |
| **A Máscara Partida** | **[Triss Merril](pc_triss_merril.md)** usa um feitiço para dissipar a escuridão. Um golpe acidental quebra a máscara de Morvanya, revelando o rosto de **Manvea**, a melhor amiga de infância de **[Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)**. | **[Triss Merril](pc_triss_merril.md)** uses a spell to dispel the darkness. An accidental hit breaks Morvanya's mask, revealing the face of **Manvea**, **[Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)'s** lost childhood best friend. |
| **O Suicídio** | Morvanya é derrotada. **[Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)** tenta curá-la e reanimá-la. Contudo, antes de poder ser interrogada, Morvanya morde uma mó de cianeto escondida e comete suicídio, morrendo nos braços de Vaunia para proteger os segredos do **[Culto do Mal Elemental](cult_of_elemental_evil.md)**. | Morvanya is defeated. **[Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)** attempts to heal and revive her. However, before she can be questioned, Morvanya bites a hidden cyanide molar and commits suicide, dying in Vaunia's arms to protect the **[Cult of Elemental Evil](cult_of_elemental_evil.md)'s** secrets. |
| **Novas Capas** | O grupo recupera a **Tapeçaria (Tapestry)** do acampamento dos cultistas. **[Markus Grimm](pc_markus_grimm.md)** usa a tapeçaria para criar **6 capas de cultista** para todos. Ele carimba a sua capa com o Carimbo de [Wee Jas](wee_jas.md). | The party recovers the **Tapestry** from the cultists' camp. **[Markus Grimm](pc_markus_grimm.md)** uses the tapestry to fashion **6 cultist cloaks** for everyone. He stamps his cloak with the [Wee Jas](wee_jas.md) Stamp. |

---
## 👥 Cast of Characters / Elenco de Personagens

| Personagem | PT | EN |
| :--- | :--- | :--- |
| **[Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)** | Vive o choque emocional da perda e da revelação, ganha um *companion* (**Boaz**). Tenta desesperadamente salvar Morvanya. | Experiences the emotional shock of loss and revelation, gains a *companion* (**Boaz**). Desperately tries to save Morvanya. |
| **[Markus Grimm](pc_markus_grimm.md)** | Usa o **Carimbo de Wee Jas** para purificar a Tapeçaria e criar as capas de *party*. | Uses the **Wee Jas Stamp** to purify the Tapestry and create the *party* cloaks. |
| **[Triss Merril](pc_triss_merril.md)** | Dissipa a escuridão, permitindo a revelação de Morvanya/Manvea. | Dispel the darkness, allowing the reveal of Morvanya/Manvea. |
| **[Dustin Thorne](pc_dustin_thorne.md)** | Cura Morvanya para permitir o interrogatório, mas ela suicida-se. Fica com o escudo. | Heals Morvanya to allow interrogation, but she commits suicide. Keeps the shield. |
| **[Acólita Morvanya](acolyte_morvanya.md)** | Líder da célula, revela-se **Manvea**, melhor amiga de Vaunia, e comete suicídio. | Cell leader, revealed to be **Manvea**, Vaunia's best friend, and commits suicide. |

---
## 💰 Rewards & Loot / Recompensas e Saque

| Item | Descrição (PT) | Description (EN) |
| :--- | :--- | :--- |
| **Capas do Culto** | Seis capas de cultista feitas da Tapeçaria (para todos, incluindo Conan e Vaunia). | Six cultist cloaks made from the Tapestry (for everyone, including Conan and Vaunia). |
| **Carimbo de Wee Jas** | Um carimbo (já na posse de Markus, mas usado nesta sessão) com o símbolo da Deusa da Morte e Magia. | A stamp (already in Markus's possession, but used this session) with the symbol of the Goddess of Death and Magic. |
| **Escudo de Cultista** | Um escudo leve (para [Dustin Thorne](pc_dustin_thorne.md)). | A light shield (for [Dustin Thorne](pc_dustin_thorne.md)). |
| **Carta de Morvanya** | Uma carta que detalha os planos futuros do Culto do Mal Elemental (para Vaunia). | A letter detailing the future plans of the Elemental Evil Cult (for Vaunia). |
| **Máscara da Morte** | Máscara prateada e partida de Morvanya (para Vaunia). | Morvanya's broken silver mask (for Vaunia). |

---
## 🧭 Aftermath & Hooks / Consequências e Ganchos

| Gancho | PT | EN |
| :--- | :--- | :--- |
| **A Nova Missão de Vaunia** | A morte de **[Manvea](manvea.md)** (Morvanya) e a perda traumática são a principal motivação de **[Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)** contra o **[Culto do Mal Elemental](cult_of_elemental_evil.md)**, especialmente contra a facção de [Nerull](nerull.md) (Deus da Morte). | The death of **[Manvea](manvea.md)** (Morvanya) and the traumatic loss are **[Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)'s** primary motivation against the **[Cult of Elemental Evil](cult_of_elemental_evil.md)**, especially against the [Nerull](nerull.md) faction (God of Death). |
| **O Plano do Culto Revelado** | A **Carta de Morvanya** (que revela os planos para [Blackfair Manor](blackfair_manor.md), a ameaça de **Borogrove** e o plano de desestabilização) torna-se o *plot guide* principal para a campanha. | **Morvanya's Letter** (which reveals the plans for [Blackfair Manor](blackfair_manor.md), the threat of **Borogrove**, and the destabilization plan) becomes the main plot guide for the campaign. |
| **Os Novos Mantos** | As capas de cultista (transformadas de Tapeçaria) dão ao grupo um disfarce e um símbolo de união, mas podem causar problemas de reputação se usadas de forma descuidada. | The cultist cloaks (made from Tapestry) give the party a disguise and a symbol of unity, but they may cause reputation problems if used carelessly. |

---
## 🔒 DM-ONLY SECTION / SEÇÃO APENAS PARA O DM

---
## 📂 Source Files / Ficheiros Fonte

| Ficheiro                                                                                                        | Tipo                        |
| :-------------------------------------------------------------------------------------------------------------- | :-------------------------- |
| [Sessão de 30 de Janeiro de 2025 andre.docx](uploaded:Sessão%20de%2030%20de%20Janeiro%20de%202025%20andre.docx) | Resumo de Sessão (Jogador)  |
| [sessao maia](uploaded:sessao%20maia)                                                                           | Notas do DM (Setup)         |
| [Documento sem nome](uploaded:Documento%20sem%20nome)                                                           | Carta de Morvanya (DM-Only) |
|                                                                                                                 |                             |

---
### 📝 Prep Notes / Notas de Preparação
* **A Manifestação:** O novo *companion* de **[Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)**, **Boaz**, é uma manifestação da sua alma fragmentada após o trauma. Ele não é um animal comum e a sua presença está ligada ao seu passado e à sua busca por memória.
* **Manvea/Morvanya:** Ela era uma agente dedicada de [Nerull](nerull.md). A quebra da máscara foi a única forma de revelar a sua verdadeira identidade. O seu suicídio foi um ato final de lealdade ao culto e de proteção do seu líder, **Lorde Aeltor** (mencionado na carta).
* **O Bonegoblin:** O Bonegoblin é o *Bone Naga* (Goblin) que está ligada ao bastão, e a sua destruição com o feitiço de [Conan o Bárbaro](pc_conan_barbaro_ra.md) foi um erro, o que permitiu que Morvanya se suicidasse.

### 🤫 Secrets & Hooks / Segredos e Ganchos
* **A Carta de Morvanya (Detalhes):** A carta (no `Documento sem nome`) revela que o Culto está a planear o caos em grande escala, incluindo o uso de goblins perto de [Two Fords](two_fords.md) e a infiltração de **Maxibom Gustgavv e Isa Merrill** na [Blackfair Manor](blackfair_manor.md) (agentes de [Nerull](nerull.md) nobreza).
* **Borogrove:** A referência ao *Treant* **Borogrove** (que protege a área) como um obstáculo a ser eliminado indica que o grupo terá de se aliar ao Treant para proteger as florestas de Greyhawk da corrupção.
* **Wee Jas vs. Nerull:** O carimbo de [Wee Jas](wee_jas.md) ser usado para *purificar* a Tapeçaria de [Nerull](nerull.md) demonstra o conflito teológico central da campanha (Morte ordeira vs. Morte Caótica/Mal Elemental).

### ⚙️ Contingencies / Contingências
* **Se Vaunia usasse o *Wild Shape*:** A sua forma seria alterada pelo trauma e pela nova ligação, manifestando padrões de pelo com fungos negros ou olhos vermelhos.
* **Se o grupo resgatasse Morvanya:** Ela seria transportada para o esconderijo de Raveth (S11), criando um triângulo de lealdade e chantagem para **[Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)**, que teria de escolher entre a amiga ou o mundo.

***

## 📊 Passo 3 — Index Maintenance / Manutenção do Índice

Com a conclusão da sessão **S05A: As scaras da Morte**, os seguintes índices teóricos seriam atualizados com a referência ao ficheiro **(s05a_mascaras_da_morte.md)**:

| Categoria         | Entidade                                            | Slug (Confirmado com docs\_structure ou Novo) | Sessões (Nova Adição) |
| :---------------- | :-------------------------------------------------- | :-------------------------------------------- | :-------------------- |
| **Missions**      | As Máscaras da Morte(s05a_mascaras_da_morte.md)     | S05A                                          |                       |
| **PCs**           | [Vaunia](docs/dm/-/pc/pc_vaunia_kolakiala.md)                    | S05A                                          |                       |
| **PCs**           | [Markus Grimm](pc_markus_grimm.md)                  | S05A                                          |                       |
| **NPCs**          | [Acólita Morvanya](acolyte_morvanya.md) (Novo)      | S05A                                          |                       |
| **NPCs**          | [Manvea](manvea.md) (Novo)                          | S05A                                          |                       |
| **NPCs**          | [Borogrove](npc_borogrove.md)                           | S05A                                          |                       |
| **Locations**     | [Mistwood](mistwood.md) (Novo)                      | S05A                                          |                       |
| **Organizations** | [Culto do Mal Elemental](cult_of_elemental_evil.md) | S05A                                          |                       |

---