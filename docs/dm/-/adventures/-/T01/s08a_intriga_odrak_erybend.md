## Intriga de Odrak em Erybend / Odrak Intrigue in Erybend

| 🗓 Timeline / Cronologia | PT | EN |
| :-------------------- | :---------------------------------------------------- | :------------------------------------------------------- |
| **Data do Jogo** | 20 de Fevereiro de 2025 | February 20th, 2025 |
| **Data In-Game** | 8 de Sunsebb, 998 CY (Semana 08) | 8th of Sunsebb, 998 CY (Week 08) |
| **Sessão #** | S08A | S08A |
| **DM** | Laura | Laura |
| **Arcos Enquadrados** | **🦑 Arco da Corrupção do Mal Elemental** | **🦑 Elemental Evil Corruption Arc** |

---
## 📖 Overview / Sumário

| Descrição | PT | EN |
| :--- | :--- | :--- |
| **Missão Principal** | A *party* é enviada a **[Erybend](erybend.md)**, uma cidade comercial estratégica, para estabelecer novos contactos e rotas comerciais, incluindo ligações a [Blackfair Manor](blackfair_manor.md) e [Two Fords](docs/locations/-/regions/two_fords.md). O contacto é **[Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)**, que alerta o grupo sobre distúrbios e raptos recentes na cidade. | The party is sent to **[Erybend](erybend.md)**, a strategic trading city, to establish new contacts and trade routes, including links to [Blackfair Manor](blackfair_manor.md) and [Two Fords](docs/locations/-/regions/two_fords.md). The contact is **[Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)**, who alerts the group to recent disturbances and kidnappings in the city. |
| **Resultado** | O grupo é emboscado por **6 indivíduos encapuzados** (cultistas) na Taberna Machado Quebrado. Após a derrota dos atacantes, o grupo recupera uma lista de 7 nomes, descobrindo que **[Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)** é o próximo alvo dos sequestros, que se suspeita serem orquestrados pelos **Cultistas de Odrak**. | The party is ambushed by **6 hooded individuals** (cultists) in the Broken Axe Tavern. After defeating the attackers, the group recovers a list of 7 names, discovering that **[Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)** is the next target of the kidnappings, which are suspected to be orchestrated by **Odrak's Cultists**. |

---
## 🎲 Key Events / Eventos Chave

| Evento | PT | EN |
| :--- | :--- | :--- |
| **O Chamado de Erybend** | **[Kythria](docs/npc/-/Free%20City%20of%20Grehawk/npc_kythria.md)**, sobrinha do Taberneiro, envia a *party* a **[Erybend](erybend.md)**, conhecida por ser uma cidade de comerciantes. No caminho, o grupo encontra o *halfling* **[Flynn](pc_flynn.md)**, que se junta à missão. | **[Kythria](docs/npc/-/Free%20City%20of%20Grehawk/npc_kythria.md)**, the Barkeep's niece, sends the *party* to **[Erybend](erybend.md)**, known as a city of merchants. En route, the group meets the *halfling* **[Flynn](pc_flynn.md)**, who joins the mission. |
| **O Aviso de Dolrea** | Na chegada, o *half-elf* **[Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)** encontra o grupo e alerta para a série de desaparecimentos e raptos noturnos, que se suspeita serem obra dos **Cultistas de Odrak**. | Upon arrival, the *half-elf* **[Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)** meets the group and warns of the series of night-time disappearances and kidnappings, suspected to be the work of **Odrak's Cultists**. |
| **Emboscada na Taberna** | O grupo segue a pista de **[Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)** até à Taberna Machado Quebrado (Broken Axe Tavern), onde veem cartazes de desaparecidos. Ao saírem, são emboscados por **6 indivíduos encapuzados** (cultistas). **[Grix Molah](pc_grix_molah.md)** compra mais de 85 pares de meias. | The party follows **[Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)'s** lead to the Broken Axe Tavern, where they see missing persons posters. As they leave, they are ambushed by **6 hooded individuals** (cultists). **[Grix Molah](pc_grix_molah.md)** buys over 85 pairs of socks. |
| **O Próximo Alvo** | Ao revistar o corpo de um dos cultistas, o grupo encontra uma lista de 7 nomes. **[Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)** está no final da lista, confirmando que os cultistas vieram raptá-lo e que ele é o próximo alvo. | Upon searching the body of one of the cultists, the party finds a list of 7 names. **[Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)** is at the bottom of the list, confirming that the cultists came to kidnap him and that he is the next target. |

---
## 👥 Cast of Characters / Elenco de Personagens

| Personagem | PT | EN |
| :--- | :--- | :--- |
| **[Kai](docs/pc/-/pc_kai_vanthir.md)** | Membro da *party* de investigação e combate em [Erybend](erybend.md). | Investigation and combat *party* member in [Erybend](erybend.md). |
| **[Grix Molah](pc_grix_molah.md)** | Envolvido na investigação do desaparecimento e na compra exagerada de meias. | Involved in the disappearance investigation and the exaggerated purchase of socks. |
| **[Varus](docs/pc/-/pc_zeldris.md)** | Participa na missão de reconhecimento comercial e no combate. | Participates in the commercial reconnaissance mission and combat. |
| **[Flynn](pc_flynn.md)** | O *halfling* que se junta à *party* na viagem. | The *halfling* who joins the *party* on the journey. |
| **[Triss Merril](pc_triss_merril.md)** | Ajuda na investigação inicial e compra meias (*mal feitas*). | Aids in the initial investigation and buys socks (*poorly made*). |
| **[Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)** | O *half-elf* local que contrata o grupo e é revelado como o próximo alvo do culto. | The local *half-elf* who contracts the group and is revealed as the cult's next target. |
| **[Kythria](docs/npc/-/Free%20City%20of%20Grehawk/npc_kythria.md)** | Sobrinha do Taberneiro que transmite a missão. | Barkeep's niece who relays the mission. |

---
## 🔒 DM-ONLY SECTION / SEÇÃO APENAS PARA O DM

### 📝 Prep Notes / Notas de Preparação
* **Cultistas de Odrak:** Estes cultistas são agentes da facção Vento/Ar do **[Culto do Mal Elemental](cult_of_elemental_evil.md)**, com foco em sequestro e coerção. Odrak é uma divindade menor ligada ao vento caótico/destrutivo.
* **Flynn:** A sua entrada na *party* foi planeada pelo Taberneiro para ter um *halfling* (uma raça normalmente neutra) com talentos druídicos na cidade.
* **A Lista:** A lista de 7 nomes é um *plot hook* direto, confirmando que os cultistas estão a trabalhar através de uma lista de alvos de alto valor (provavelmente comerciantes com ligações à Taberna).

### 🤫 Secrets & Hooks / Segredos e Ganchos
* **O Próximo Alvo:** **[Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)** é um mercador chave que, se for raptado, perturbará o fluxo de mercadorias para o Norte e para a Taberna, beneficiando o Culto. A sua proteção é vital.
* **O Templo:** O movimento estranho no templo na periferia de [Erybend](erybend.md) (mencionado por Dolrea) é a base de operações da célula do culto. O templo é o destino dos raptados para rituais e/ou interrogatório.
* **Odrak:** Este culto menor é subserviente a **[Nerull](docs/dm/-/organizations/-/cults/cult_of_nerull.md)** ou a um líder do Mal Elemental superior. O seu objetivo é o caos através de sequestro e controlo.

### ⚙️ Contingencies / Contingências
* **Se Dolrea fosse raptado:** A missão falhava e o grupo teria de invadir o templo para o resgatar (Missão S11B), enfrentando um número muito maior de cultistas.
* **Se o grupo ignorasse o rapto:** Os raptos continuariam e os cultistas sentiriam-se à vontade para operar abertamente, levando a um controlo total de [Erybend](erybend.md) pelo [Culto do Mal Elemental](cult_of_elemental_evil.md) (facção Vento/Ar).

---
## 📂 Source Files / Ficheiros Fonte

| Ficheiro | Tipo |
| :--- | :--- |
| [resumo - sessão 20 fevereiro.pdf](uploaded:resumo%20-%20sessão%2020%20fevereiro.pdf) | Resumo de Sessão (Jogador) |

---
## 💰 Rewards & Loot / Recompensas e Saque

| Item | Descrição (PT) | Description (EN) |
| :--- | :--- | :--- |
| **Lista de Nomes** | Lista de 7 alvos de rapto do culto (inclui [Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)). | List of 7 cult kidnapping targets (includes [Dolrea Vas](docs/npc/-/Eribend/npc_dolrea_vas.md)). |
| **Meias** | **[Grix Molah](pc_grix_molah.md)** compra mais de 85 pares de meias. | **[Grix Molah](pc_grix_molah.md)** buys over 85 pairs of socks. |

---

## 📊 Passo 3 — Index Maintenance / Manutenção do Índice

Com a conclusão da sessão **S08A: Intriga de Odrak em Erybend**, os seguintes índices teóricos seriam atualizados com a referência ao ficheiro **(s08a_intriga_odrak_erybend.md)**:

| Categoria         | Entidade                                                      | Slug (Confirmado com docs\_structure ou Novo) | Sessões (Nova Adição) |
| :---------------- | :------------------------------------------------------------ | :-------------------------------------------- | :-------------------- |
| **Missions**      | [Intriga de Odrak em Erybend ](docs/dm/-/adventures/-/S01/s08a_intriga_odrak_erybend.md) | S08A                                          |                       |
| **PCs**           | [Kai](docs/pc/-/pc_kai_vanthir.md)                                              | S08A                                          |                       |
| **PCs**           | [Grix Molah](pc_grix_molah.md)                                | S08A                                          |                       |
| **PCs**           | [Varus](docs/pc/-/pc_zeldris.md)                                        | S08A                                          |                       |
| **PCs**           | [Flynn](pc_flynn.md)                                          | S08A                                          |                       |
| **PCs**           | [Triss Merril](pc_triss_merril.md)                            | S08A                                          |                       |
| **NPCs**          | [Dolrea Vas ](docs/npc/-/Eribend/npc_dolrea_vas.md) (Novo)                           | S08A                                          |                       |
| **NPCs**          | [Kythria ](docs/npc/-/Free%20City%20of%20Grehawk/npc_kythria.md) (Novo)                                 | S08A                                          |                       |
| **Organizations** | [Cultistas de Odrak ](cultists_of_odrak.md) (Novo)            | S08A                                          |                       |
| **Locations**     | [Erybend ](erybend.md) (Novo)                                 | S08A                                          |                       |

---
