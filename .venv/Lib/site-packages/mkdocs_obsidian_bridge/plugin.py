...
class ObsidianBridgePlugin(BasePlugin[ObsidianBridgeConfig]):
    ...
    def best_path(self, page_dir: Path, candidates: list[Path]) -> Path:
        """Return the shortest candidate path relative to page_dir, cross-drive safe on Windows."""
        ...
        if len(candidates) == 1:
            ...
            b = page_dir
            try:
                # same drive → relative; different drives → absolute
                if os.name == "nt" and getattr(a, "drive", "").lower() != getattr(b, "drive", "").lower():
                    return a
        ...
        for c in candidates:
            try:
                if os.name == "nt" and getattr(c, "drive", "").lower() != getattr(page_dir, "drive", "").lower():
                    rel_paths.append(c)  # keep absolute if on another drive
                else:
                    rel_paths.append(Path(os.path.relpath(c, page_dir)))
            except Exception:
                rel_paths.append(c)
